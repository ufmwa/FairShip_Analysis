...
if _TORCH_AVAILABLE:
    ...
    def _nested_concatenate(input_graphs, field_name, axis):
        features_list = [
            getattr(gr, field_name) for gr in input_graphs if getattr(gr, field_name) is not None
        ]
    ...
    class EncodeProcessDecode(nn.Module):
        ...
        def __init__(
            ...
            self._encoder = MLPGraphIndependent(
                mlp_output_size, mlp_output_size, mlp_output_size
            )

            self._blocks = []
            for _ in range(num_blocks):
                core = MLPGraphNetwork(
                    edge_output_size=mlp_output_size,
                    node_output_size=mlp_output_size,
                    global_output_size=mlp_output_size,
                )
                self._blocks.append(core)
    ...
    class EncodeProcessDecode:
        def __init__(self, *args, **kwargs):
            raise RuntimeError(
                "EncodeProcessDecode requires torch; caller should have gated this path."
            )
