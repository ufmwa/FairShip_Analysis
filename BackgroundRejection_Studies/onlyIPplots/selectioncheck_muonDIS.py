...
def main(weight_function,IP_CUT = 250,fixTDC=None,fix_candidatetime=None):
    ...
    for c in cats:
        ...
        ut.bookHist(hist_dict[c], pre+"impact_parameter_vs_zvtx", "Impact parameter vs z-vtx; cm", 300, 0, 300,6000, -3000, 3000)
        ut.bookHist(hist_dict[c], pre+"impact_parameter_vs_z_interac_point", "Impact parameter vs z-interaction point; cm", 300, 0, 300,6000, -3000, 3000)
    ...
    for event_nr, event in enumerate(tree):
        ...
        for c in {"all", cat}:
            ...
            if c not in scalefactor:            
                scalefactor[c] = ndis_rescale(options.jobDir,event_nr,c)
        ...
        for candidate_id_in_event, signal in enumerate(event.Particles):
            ...
            #if selection_list['GoodDaughters'+'+n_particles'+'+fiducial'+'+doca'] and cat=='vesselCase':
                #print(event_weight * scalefactor['vesselCase'])
            ...
            for c in {"all", cat}:
                ...
                hist_dict[c][pre+"impact_parameter_vs_z_interac_point"        ].Fill( selection.impact_parameter(signal),interaction_point.Z(),event_weight_rescaled)   
                ...
                hist_dict[c][pre+"impact_parameter_vs_zvtx"        ].Fill( selection.impact_parameter(signal),signal_pos.Z(),event_weight_rescaled)   
